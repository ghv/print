...
// Put this in one key so that we can grant permissions to just one item in Keychain
public struct AWSCredentials: Codable {
...
public class KeychainAccess {
    ...
    static let printServiceName: AnyObject = "com.ghvxp.print" as AnyObject
    ...
    static func save(item: String, value: Data) throws {
        let query: [String: AnyObject] = [
            // kSecAttrService,  kSecAttrAccount, and kSecClass
            // uniquely identify the item to save in Keychain
            kSecAttrService as String: printServiceName,
            kSecAttrAccount as String: item as AnyObject,
    ...
    static func update(item: String, value: Data) throws {
        let query: [String: AnyObject] = [
            // kSecAttrService,  kSecAttrAccount, and kSecClass
    ...
    public static func write(item: String, value: Data) throws {
        do {
            try save(item: item, value: value)
        } catch KeychainError.duplicateItem {
            try update(item: item, value: value)
        }
    }

    public static func read(item: String) throws -> Data {
        let query: [String: AnyObject] = [
            // kSecAttrService,  kSecAttrAccount, and kSecClass
